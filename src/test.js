...
class DeviceLog extends PureComponent {
  ...
  componentWillMount() {
    ...
    if (view && stack) {
      getConfig(
        utils.fillInPath("/device_configs/{id}/log/override", view, stack)
      );
            ...
            <Switch
              ...
              onChange={(checked) => {
                Modal.confirm({
                  ...
                  onOk() {
                    doConfig({
                      entry: utils.fillInPath(
                        `/device_configs/{id}/log_override/${checked}`,
                        view,
                        stack
                      ),
                      action: "update",
                      ...
                      onSuccess: () => {
                        getConfig(
                          utils.fillInPath(
                            "/device_configs/{id}/log/override",
                            view,
                            stack
                          )
                        );
        ...
        {utils.isUnavailable(view, stack) ? (
          ...
          <DataForm
            {...{
              view,
              values: {
                ..._.get(config, ["entries", this.state.path, "result"], ""),
              },
